...
if ! [ -d "/Library/Application Support/MCC-DistributedNotifications" ]
    then
        mkdir "/Library/Application Support/MCC-DistributedNotifications"
...
/bin/sh "$CURRENTDIR/uninstall.sh"
...
if ! [ -f "$CURRENTDIR/LaunchAgent-Daemon.app" ]
    then
...
cp "$CURRENTDIR/de.miralem-cebic.launchagent.plist" "/Library/LaunchAgents/de.miralem-cebic.launchagent.plist"
cp "$CURRENTDIR/de.miralem-cebic.launchdaemon.plist" "/Library/LaunchDaemons/de.miralem-cebic.launchdaemon.plist"

echo "\n"
echo "--------------"
echo "->COPY APPS"

cp -R "$CURRENTDIR/LaunchAgent-Daemon.app" "/Library/Application Support/MCC-DistributedNotifications/LaunchAgent-Daemon.app"
cp -R "$CURRENTDIR/LaunchDaemon-Daemon.app" "/Library/Application Support/MCC-DistributedNotifications/LaunchDaemon-Daemon.app"
